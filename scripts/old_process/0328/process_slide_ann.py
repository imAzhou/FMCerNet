...
def cut_patchlist(total_slideInfo):
    pos_save_dir = 'data_resource/0328/0410slide/Pos'
    os.makedirs(pos_save_dir, exist_ok=True)
    neg_save_dir = 'data_resource/0328/0410slide/Neg'
...
def gene_patchinfo(patch_coords,patch_filename,pos_rect_items):
    ...
    for rect in pos_rect_items:
        ...
        if coord is not None:
            ...
            patchinfo['prefix'] = '0410slide/Pos'
...
def gene_ann_json():
    ...
    test_csv_file = 'data_resource/slide_anno/0319/test_posneg.csv'
    ...
    for clsname,slidelist in annotaion.items():
        for slideinfo in tqdm(slidelist, ncols=80, desc=f'Processing {clsname}'):
            ...
            filtered = df_test.loc[df_test['kfb_path'] == kfb_path].iloc[0]
            
            new_train_rows.append(filtered)
            df_test = df_test[df_test['kfb_path'] != kfb_path].reset_index(drop=True)
