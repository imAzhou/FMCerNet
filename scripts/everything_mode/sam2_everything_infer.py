...
def everything_infer_test():
    ...
    mask_generator = get_sam_autopredictor()
    ...
    for imgpath in tqdm(demo_paths, ncols=80):
        read_image = Image.open(f'{img_root_dir}/imgpath{imgpath}')
...
def everything_infer(proc_id, device, json_save_dir, set_group):
    ...
    for idx, img_path in enumerate(set_group):
        ...
        # image_with_masks = show_anns_on_image(read_image.convert("RGBA"), masks)
        # image_with_masks.save(f"{mask_save_dir}/{filename}")
        with open(f'{json_save_dir}/{prefix}/{purename}.json', 'w', encoding='utf-8') as f:
            json.dump(masks, f, ensure_ascii=False)
...
def main():
    ...
    all_imgpath = glob.glob(f'{data_root}/images/**/*.png')
...
def test_infer():
    root_dir = 'data_resource/everything_mode'
    mask_save_dir = f'{root_dir}/test_infer'
    os.makedirs(mask_save_dir, exist_ok=True, mode=0o777)

    data_root = '/c22073/zly/datasets/CervicalDatasets/WINDOW_SIZE_1000'
    all_jsonpath = glob.glob(f'{data_root}/sam2Infer/**/*.json')
    for json_path in tqdm(all_jsonpath, ncols=80):
        with open(json_path, 'r', encoding='utf-8') as f:
            masks = json.load(f)

        filename = json_path.split('/')[-1]
        prefix = json_path.split('/')[-2]
        img_path = json_path.replace('sam2Infer', 'images').replace('.json', '.png')
        read_image = Image.open(img_path)
        purename = filename.replace('.json', '')
