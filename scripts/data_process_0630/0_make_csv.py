...
def make_zheyi_pos():
    ...
    with open('data_resource/zheyi_annofiles/宫颈液基细胞—Slide-0607.json', 'r', encoding='utf-8') as f:
        slide0607_json_data = json.load(f)
        del slide0607_json_data['NILM'] # 暂时不处理阴性 slide 的标注信息
    ...
    for slide_annjson in [slide0409_json_data, slide0422_json_data, slide0607_json_data]:
        for clsname,slidelist in slide_annjson.items():
            for slideinfo in tqdm(slidelist, ncols=80):
                ...
                if patientId != '': # 没查到 patientId 是因为该文件已经被废弃
    ...
    df_total_pos.to_csv(f'data_resource/0630/0_zheyi_pos.csv', index=False)
