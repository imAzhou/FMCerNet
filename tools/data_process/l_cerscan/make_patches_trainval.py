...
def main(use_source3):
    # with open(f'data_resource/WINDOW_SIZE_{WINDOW_SIZE}/ann_jsons/ppatches_in_NegSlide.json', 'r', encoding='utf-8') as f:
    ...
    RoI_patchlist = filter_slide_neg(RoI_patchlist, neg_patch_thr=neg_patch_thr) # Limit negative patches per patient slide.
    ...
    data_group = {
        'puretrain': 'data_resource/4_pure_train.csv',
        'val': 'data_resource/6_val.csv'
    }
    for tag,csvpath in data_group.items():
        ...
        if tag == 'puretrain' and use_source3:
            # with open(f'{data_root}/ann_jsons/source3train_patches_in_NegSlide.json', 'r', encoding='utf-8') as f:
            #     source3train_negslide_patchlist = json.load(f)
            with open(f'{data_root}/ann_jsons/patches_in_RoI_source3_valid.json', 'r', encoding='utf-8') as f:
                source3_pos_patchdata = json.load(f)
            df_source3train = pd.read_csv('data_resource/5_source3_train.csv')
            # source3_patchdata = [*source3train_negslide_patchlist, *source3_pos_patchdata]
            source3_patchdata = source3_pos_patchdata
            source3_patchdata = [i for i in source3_patchdata if i['patientId'] in list(df_source3train['patientId'])]

            fusionPatchInCOCO = coco_format([*patchlist, *source3_patchdata])
...
def draw_dataset_gt():
    ...
    for imgitem in tqdm(patch_COCOinfo['images'], ncols=80):
        ...
        if filename != 'SOURCE2_1_25_1717869845996_0.png':
            continue
