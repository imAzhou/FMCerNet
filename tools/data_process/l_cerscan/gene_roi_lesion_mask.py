...
from cerwsi.utils import random_cut_square,calc_relative_coord,is_bbox_inside,set_seed,generate_cut_regions
...
test_patientId = [
    'SOURCE1_1_45', 'SOURCE1_1_184', 'SOURCE2_1_89',   # Example slides from 0409, 0422, and 0607.
    'SOURCE3_2_1486', 'SOURCE3_2_152', 'SOURCE3_2_239',     # Empty RoI and annotated RoI examples.
    'SOURCE2_1_26',    # partial_pos slide in pure_train
    'SOURCE3_2_2111', 'SOURCE3_2_2',     # partial_pos slide in source3_train
    'SOURCE3_2_302',
]
...
def vis_sample(image, roi_masks, roi_item, savepath):
    plt.figure(figsize=(20, 20))
    plt.imshow(image)

    rx1, ry1, rx2, ry2 = roi_item['region']

    # 1. Draw red dashed boxes from children annotations.
    ...
    # 2. Draw masks and their enclosing boxes.
    annids = np.unique(roi_masks)
    for annid_idx in annids[1:]:  # The first ID is background 0.
        mask = roi_masks == annid_idx
        # show_mask(mask, plt.gca(), random_color=True)
        plt.contour(mask, levels=[0.5], colors='lime', linewidths=2)

        # Enclosing box with a solid green line.
        yx = np.argwhere(mask)
        if yx.size > 0:
...
def postprocess_mask(masks, input_boxes, input_boxes_annid, sort_by_area=True):
    ...
    Drop an input bbox when it is contained by another bbox and its mask
    substantially overlaps with the parent-box mask.
    '''
...
def gene_roi_lesion_mask(proc_id, all_json_datas, npz_mask_save_dir):
    ...
    for idx, item in enumerate(all_json_datas): # Patient-level loop.
        ...
        if item['media_type'] == 'roi':
            ...
            slide = open_wsi(item['source_path'])

        for RoIItem in item['annotations']: # RoI-level loop.
            ...
            try:
                ...
                # Sort by bbox area in descending order.
                ...
                for annitem in RoIItem['children']: # Bbox-level loop.
                    ...
                    for i, (mask, annid) in enumerate(zip(masks, input_boxes_annid)):
                        ...
                        ys, xs = np.where(mask)     # Coordinates where the current mask is True.
                        annid_idx = annid_list.index(annid) + 1     # Indices start from 1; 0 is background.
                ...
                if len(RoIItem['children']) > 0:
                    sparse_mask = sparse.coo_matrix(roi_mask)  # Store only nonzero positions and values.
...
def test_roi_lesion_mask(all_json_datas,npz_mask_save_dir):
    for idx, item in enumerate(tqdm(all_json_datas, ncols=80)):
        # if idx != 1585:
        #     continue
        # if item['patientId'] != 'SOURCE1_1_1479':
        #     continue
        if item['patientId'] not in test_patientId:
            continue
        if item['media_type'] == 'roi':
        ...
        for RoIItem in item['annotations']:
            ...
            savedir = f'statistic_results/0630/sam2_infer_RoI'
            os.makedirs(savedir, exist_ok=True, mode=0o777)
            savepath = f'{savedir}/{purename}.png'
            vis_sample(sample_img, roi_mask, RoIItem, savepath)
            draw_cnt += 1

def visual_huge_roi(all_json_datas,npz_mask_save_dir):
    for idx, item in enumerate(tqdm(all_json_datas, ncols=80)):

        if item['patientId'] != 'SOURCE2_1_25':
            continue
        ...
        for RoIItem in item['annotations']:
            ...
            if abs(rw-ws) < 100 or abs(rh-ws) < 100:
                random_x1,random_y1 = random_cut_square([0,0,rw,rh],ws)  # Relative coordinates in the RoI.
            ...
            for iidx,rect_coords in enumerate(cut_points):
                ...
                new_RoIItem['children'] = [i for i in new_RoIItem['children'] if is_bbox_inside(i['region'],new_RoIItem['region'],tolerance=10)]
                if len(new_RoIItem['children']) == 0:
                    continue
                new_roi_mask = roi_mask[start_y1:start_y2, start_x1:start_x2]

                if item['media_type'] == 'roi':
                    sample_img = roi_img.crop([media_x1,media_y1,media_x2,media_y2])
                elif item['media_type'] == 'slide':
                    location, level, size = (media_x1,media_y1), 0, (media_x2-media_x1,media_y2-media_y1)
                    sample_img = Image.fromarray(slide.read_region(location, level, size))

                print(f'\nDrawing {purename}_{iidx}.png, (width,height) is {sample_img.size}')
                savedir = f'statistic_results/0630/sam2_infer_RoI/{item["patientId"]}'
                os.makedirs(savedir, exist_ok=True, mode=0o777)
                savepath = f'{savedir}/{str(RoIItem["annid"])}_{iidx}.png'
                vis_sample(sample_img, new_roi_mask, new_RoIItem, savepath)
...
def statistic_roi_anncnts(all_json_datas,npz_mask_save_dir):
    ...
    for idx, item in enumerate(tqdm(all_json_datas, ncols=80)):
        for RoIItem in item['annotations']:
            ...
            rw,rh = int(rx2-rx1+0.5), int(ry2-ry1+0.5)
            purename = item['patientId'] + f'_{str(RoIItem["annid"])}'
            # if purename != 'SOURCE3_2_88_5205493336944':
            #     continue
...
def test_file_exist(all_json_datas,npz_mask_save_dir):
    ...
    for idx, item in enumerate(tqdm(all_json_datas, ncols=80)):
        # if item['patientId'] != 'SOURCE2_3_624':
        #     continue
        for RoIItem in item['annotations']:
            purename = item['patientId'] + f'_{str(RoIItem["annid"])}'
...
if __name__ == "__main__":
    set_seed(666)
    with open('data_resource/0630/source1_roi.json', 'r', encoding='utf-8') as f:
        source1_roi_data = json.load(f)
    with open('data_resource/0630/source1_slide.json', 'r', encoding='utf-8') as f:
        source1_slide = json.load(f)
    with open('data_resource/0630/source2_pos_slide.json', 'r', encoding='utf-8') as f:
        source2_pos_slide = json.load(f)
    with open('data_resource/0630/source3_pos_slide.json', 'r', encoding='utf-8') as f:
        source3_pos_slide = json.load(f)

    all_json_datas = [*source1_roi_data, *source1_slide, *source2_pos_slide, *source3_pos_slide]
    npz_mask_save_dir = 'data_resource/0630/roi_inst_mask'
    os.makedirs(npz_mask_save_dir, exist_ok=True, mode=0o777)
    
    # Test single-ROI mask generation and skip ROIs with existing masks.
    ...
    #         Flag = True
    #     if Flag:
    #         nan_exise.append(item)
    # all_json_datas = nan_exise
    ...
    visual_huge_roi(all_json_datas, npz_mask_save_dir)
    # test_file_exist(all_json_datas,npz_mask_save_dir)
    # clear_npz(all_json_datas,npz_mask_save_dir)
    # statistic_roi_anncnts(all_json_datas, npz_mask_save_dir)    # 39447/40844, 49099/50566
