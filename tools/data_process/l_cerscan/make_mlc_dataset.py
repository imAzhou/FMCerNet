...
max_negpatch_nums = 15    # Maximum negative tiles per slide; -1 keeps all.
max_try = 100   # Constraint: neg_patch_thr <= max_try; maximum sampling attempts on each negative slide.
...
def cut_negslide():
    ...
    for tag,csvpath in data_group.items():
        ...
        df = df[df['slide_clsid']==0] # Keep negative slides.
        data_list = df.to_dict(orient="records")  # Convert each row to a dict.

        # ---- Split data so each GPU processes distinct samples. ----
        rank = args.rank
        ...
        for idx,row in enumerate(data_per_rank):
            wsi_path, patientId = row["wsi_path"], row["patientId"]
            ...
            while slide_patch_cnt < max_negpatch_nums and try_cnt < max_try:
                ...
                if max(outputs[0].pred_score) > CERTAIN_THR and outputs[0].pred_label == 1:
                    ...
                    neg_patch_list.append({
                        'patientId': patientId,
                        'filename': filename,
                        'square_coords': (x1,y1,x1+WINDOW_SIZE,y1+WINDOW_SIZE),    # Relative coordinates in the media resource.
                        'bboxes': [],
                        'clsnames': [],
                        'prefix': 'neg_slide',
                        'diagnose': 0,
...
def statistic():
    for tag in ['puretrain','val']:
        ...
        # Class index -> number of tiles containing that class.
