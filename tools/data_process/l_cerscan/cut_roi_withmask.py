...
test_patientId = [
    'SOURCE1_1_45', 'SOURCE1_1_21', 'SOURCE1_1_1481',   # 0409 Slide, 0422 Slide, 0607 Slide
    # 'SOURCE3_2_1486', 'SOURCE3_2_152',     # Empty RoI and annotated RoI examples.
    # 'SOURCE3_1_2', 'SOURCE3_2_2111', 'SOURCE2_1_26'     # partial_pos slide 'SOURCE3_2_133', 
]
...
def gene_patch_jsonlist(proc_id, all_json_datas, npz_mask_save_dir, patch_npz_save_dir):
    ...
    for idx, item in enumerate(all_json_datas):
        ...
        for RoIItem in item['annotations']:
            ...
            RoI_patch_idx = 0
            # cut_points use left-closed, right-open coordinates: [0, w/h).
            if abs(rw-WINDOW_SIZE) < 100 or abs(rh-WINDOW_SIZE) < 100:
                random_x1,random_y1 = random_cut_square([0,0,rw,rh],WINDOW_SIZE)  # Relative coordinates in the RoI.
            ...
            for iidx,rect_coords in enumerate(cut_points):
                x1,y1,x2,y2 = rect_coords    # Relative coordinates in the RoI.
                ...
                pItem = {
                    ...
                    'square_coords': [x1+rx1,y1+ry1,x2+rx1,y2+ry1],    # Relative coordinates in the media resource for image cropping.
                    'filename': f'{purename}_{RoI_patch_idx}.png',
                    'bboxes': bboxes,    # Relative coordinates in the patch.
                    'clsnames': clsnames,
                }
                ...
                if len(bboxes) == 0:
                    ...
                    # slide = open_wsi(pItem['source_path'])
...
def calc_patch_anns(patch_coords, RoIItem, roi_mask):
    rpx1, rpy1, rpx2, rpy2 = patch_coords  # Coordinates relative to the ROI.
    ...
    # Use find_objects to get slices for all nonzero instances.
    objects = find_objects(patch_mask)  # Returns one slice per nonzero label value.
    for aidx in annidx[1:]:  # Skip background 0.
        ...
        # Identify small objects near patch borders.
        ...
        new_patch_mask[yslice, xslice][region] = len(ann_bboxes)  # Instance IDs start from 1.
...
def process_cut(proc_id, img_save_dir, patchlist:dict):
    ...
    for (keyname, patchlist),idx in zip(patchlist.items(), range(total_nums)):
        ...
        for patchinfo in patchlist:
            ...
            if patchinfo['prefix'] == 'neg':    # Patches without positive boxes still need valid-patch filtering.
...
if __name__ == "__main__":
    ...
    with open(f'data_resource/{timetag}/source1_roi.json', 'r', encoding='utf-8') as f:
        source1_roi_data = json.load(f)
    with open(f'data_resource/{timetag}/source1_slide.json', 'r', encoding='utf-8') as f:
        source1_slide = json.load(f)
    with open(f'data_resource/{timetag}/source2_pos_slide.json', 'r', encoding='utf-8') as f:
        source2_pos_slide = json.load(f)
    with open(f'data_resource/{timetag}/source3_pos_slide.json', 'r', encoding='utf-8') as f:
        source3_pos_slide = json.load(f)

    source1_pos_slide = [*source1_roi_data, *source1_slide, *source2_pos_slide]
    for all_json_datas,patches_jsonname in zip([source1_pos_slide, source3_pos_slide], ['patches_in_RoI_pure', 'patches_in_RoI_source3']):
