...
def apply_cutout_mask(
    ...
    for _ in range(num_holes):
        ...
        mask[0, y1:y1 + hole_h, x1:x1 + hole_w] = 0
    return mask
...
class WSCerPartial(MetaClassifier):
    def __init__(self, args):
        ...
        self.apply_auxiliary = args.apply_auxiliary
        self.token_cls_branch = TokenClsBranch(num_classes, input_embed_dim)
        self.binary_cls_branch = BinaryClsBranch(input_embed_dim)
    #     if self.apply_auxiliary == 'logit':
    #         self.freeze_binary_cls_branch()

    # def freeze_binary_cls_branch(self):
    #     for p in self.binary_cls_branch.parameters():
    #         p.requires_grad = False

    def calc_logits(self, img_tokens: torch.Tensor, databatch, apply_auxiliary: str|None):
        '''
        ...
        if apply_auxiliary == 'random':
            for i in range(bs):
                # prefix = databatch['metainfo'][i]['prefix']
                auxiliary_masks[i,...] = (apply_cutout_mask((1,H,W))).to(img_tokens.device)
        elif apply_auxiliary == 'logit':
            auxiliary_masks = torch.sigmoid(mask_logits).unsqueeze(1)   # (bs,1,H,W)
        
        img_logits = self.binary_cls_branch(img_tokens, auxiliary_masks)
        
        if apply_auxiliary == 'logit':
            auxiliary_masks = (auxiliary_masks < 0.5).int() # 1: 背景/阴性区域，保留，0：病变区域，掩盖
        return mask_logits, token_logits, img_logits, auxiliary_masks
     
    def create_feat_gt(self, auxiliary_masks, databatch, apply_auxiliary, device):
        '''
        auxiliary_masks:  (bs,1,H,W),  1 值区域保留mask gt，0 值区域丢弃 mask gt
        '''
        ...
        for i in range(bs):
            prefix = databatch['metainfo'][i]['prefix']
            if prefix == 'partial_pos' and aux_mul_mask[i] > 0:
                img_gt[i] = 1
            elif prefix == 'neg':
                img_gt[i] = 0
            elif prefix == 'total_pos':
                img_gt[i] = 0 if aux_mul_mask[i] == 0 else 1
    ...
    def calc_loss(self, feature_emb, databatch):
        ...
        # 计算logits与gt
        mask_logits, token_logits, img_logits, auxiliary_masks = self.calc_logits(feature_emb, databatch, self.apply_auxiliary)
        mask_gt, token_gt, img_gt, balanced_mask = self.create_feat_gt(auxiliary_masks, databatch, self.apply_auxiliary, self.device)
    ...
    def set_pred(self,feature_emb, databatch):
        mask_logits, token_logits, img_logits, auxiliary_masks = self.calc_logits(feature_emb,databatch, None)
