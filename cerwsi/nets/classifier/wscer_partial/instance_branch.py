    ...
    def loss(self, dict_inputs: dict, databatch, mask_input=None):
        ...
        for i in range(num_imgs):
            ...
            instance_mask = F.interpolate(
                gt_instances.masks.unsqueeze(1),        # (num_gts, 1, h, w)
                size=(logit_h, logit_w), mode='nearest').squeeze(1)  # (num_gts, H, W)
            gt_instances.masks = instance_mask.float()
            batch_gt_instances.append(gt_instances.to(device))
